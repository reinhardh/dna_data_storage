...
public:
	...
	polynomial& operator += (const polynomial& x){
		for(unsigned i = 0;i != min(poly.size(),x.poly.size()); i++)
			poly[i] += x.poly[i];
	...
	polynomial& operator *= (const polynomial& x){
		vector<coeff_t> tmp(x.poly.size()+poly.size()-1,0);
		for(unsigned i=0;i!=poly.size();++i){
 ...
 template<class coeff_t>
    ostream &operator<<(ostream &stream, polynomial<coeff_t>  x)
    {
...
pair<polynomial<co_t>,polynomial<co_t> > divide(const polynomial<co_t>& a, const polynomial<co_t>& b){
	...
	if(b.degree() > a.degree()){ // then a is not divisible by b, and quot = 0
		quot.poly.resize(1);
		quot.poly[0] = co_t(0);
		return pair<polynomial<co_t>,polynomial<co_t> >(quot,rem);
	}
