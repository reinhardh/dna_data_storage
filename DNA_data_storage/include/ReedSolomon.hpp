...
class RScode{
	...
	void RSencode(const vector<GFE>& infvec, vector<GFE>& c);
	
	// endode a shortened RS code 
	// infvec: contains the information, length k
	// n: the length of cw
	// n_u: length of the underlying RS code 
	void RS_shortened_encode(const vector<GFE>& infvec, vector<GFE>& c);

	// GF: coefficients of the RS code, GF(p^m)
	// infvec: vector with k entries, containing the information
	void RS_systematic_encode(const vector<GFE>& infvec, vector<GFE>& c);
...
template<class GFE, class Dft > 
void RScode<GFE,Dft>::RSencode(const vector<GFE>& infvec, vector<GFE>& c){
...
template<class GFE,class Dft>
void RScode<GFE,Dft>::RS_shortened_encode(const vector<GFE>& infvec, vector<GFE>& c){
...
// infvec: vector with k entries, containing the information
template<class GFE,class Dft>
void RScode<GFE,Dft>::RS_systematic_encode(const vector<GFE>& infvec, vector<GFE>& c){
...
pair<unsigned,unsigned> RScode<GFE,Dft>::RScode<GFE,Dft>::RSdecode(vector<GFE>& crec, const vector<GFE>& c){
	...
	for(unsigned i=0;i<er_pos.size(); ++i){
		vector<GFE> prov(2);
		/////// here I loose time when computing the powers.. 
